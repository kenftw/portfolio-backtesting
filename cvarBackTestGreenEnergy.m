...
clear
clc
 % data is in this order: ORA REGI BLDP ENPH FSCR GPRE OPTT SPWR
format long

Short = 0; % if Short=1, we allow for short positions; if Short = 0, we don't.
numScenarios = 15; %number of scenarios when creating Rs linspace... determines the # of portfolios along the efficient frontier

%% Reading in and cleaning the data, clearing the NaN rows, 26 NaN rows found in data set of 4480
%read data into 3 arrays from stockData.xlsx containing data from
%01/04/2000 - 12/31/2017
[num,text,raw] = xlsread('energyGreen_2012BGN.xlsx');
...
e = ones((size(mu)));
e = e';
tm = 1/(e'*(Sigma\(mu' - Rf*e)));
mktPort = tm*(Sigma\(mu'-Rf*e));
mktPortReturn = mu*mktPort;
mktPortStdDev = sqrt(tm*(mktPortReturn - Rf));
beta = SD'./mktPortStdDev;
CML = Rf + (mktPortReturn - Rf)*beta;
...
%plot(CVarwm,beta,'k','Linewidth',1.5)
set(gca,'Box','on','LineWidth', 1.5, 'FontSize',12')
grid on
...
SharpeMarkowitz = sharpe(portfolioReturnsMarkowitz,0.00)
SharpeCVAR = sharpe(portfolioReturnsCVAR,0.00)
SharpeMarkowitzP10 = sharpe(portfolioReturnsMarkowitzP10,0.00)

figure(5)
plot(portfolioReturnsMarkowitz)
title('Min Var Portfolio returns (Green)')
xlabel('Month')
ylabel('Rate of Return')
meanCVAR = mean(portfolioReturnsCVAR);
stCVAR =  std(portfolioReturnsCVAR);

annualizedSharpe = SharpeMarkowitz * sqrt(12);
